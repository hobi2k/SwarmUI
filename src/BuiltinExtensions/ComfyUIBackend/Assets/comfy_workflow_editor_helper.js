...
/**
 * Recursively removes transient values from workflow state that Swarm's embedded frontend cannot safely restore.
 */
function comfySanitizeWorkflowValue(value) {
    if (typeof value === 'string') {
        return comfyRgthreeTempUrlPattern.test(value) ? '' : value;
    }
    if (Array.isArray(value)) {
        for (let i = 0; i < value.length; i++) {
            value[i] = comfySanitizeWorkflowValue(value[i]);
        }
        return value;
    }
    if (value && typeof value === 'object') {
        for (let [key, child] of Object.entries(value)) {
            value[key] = comfySanitizeWorkflowValue(child);
        }
    }
    return value;
}
...
/**
 * Makes a workflow JSON blob safe for Swarm's embedded Comfy frontend by stripping app-mode-only metadata.
 */
function comfyPrepareWorkflowForSwarm(rawWorkflow) {
    let workflow = typeof rawWorkflow === 'string' ? JSON.parse(rawWorkflow) : rawWorkflow;
    workflow = comfyFrame().contentWindow.LiteGraph.cloneObject(workflow);
    workflow = comfySanitizeWorkflowValue(workflow);
    if (workflow?.extra && typeof workflow.extra === 'object') {
        delete workflow.extra.linearData;
        delete workflow.extra.linearMode;
    }
    return workflow;
}
                ...
                comfyEnableInterval = null;
                    ...
                    for (let node of workflow.nodes) {
	                    ...
	                    for (let color of Object.keys(nodeColorMap)) {
	                        ...
	                        let newPrompt = {};
	                        let canSplitPrompt = true;
	                        function addAncestors(nodeId) {
	                            if (!canSplitPrompt || newPrompt[nodeId]) {
	                                return;
	                            }
	                            let promptNode = output[nodeId];
	                            if (!promptNode) {
	                                console.warn(`Swarm multi-GPU prompt splitter could not find prompt node ${nodeId}; falling back to normal queueing for this workflow.`);
	                                canSplitPrompt = false;
	                                return;
	                            }
	                            newPrompt[nodeId] = promptNode;
	                            for (let input of Object.values(promptNode.inputs || {})) {
...
function comfyLoadByName(name) {
    ...
    genericRequest('ComfyReadWorkflow', { 'name': name }, (data) => {
        let workflow = comfyPrepareWorkflowForSwarm(data.result.workflow);
        comfyFrame().contentWindow.app.loadGraphData(workflow);
...
function comfySaveModalSaveNow() {
    ...
    let doSave = (image) => {
        ...
        comfyBuildParams(false, (params, prompt_text, retained, paramVal, workflow) => {
            ...
            workflow = comfyPrepareWorkflowForSwarm(workflow);
...
function comfyImportWorkflow() {
    genericRequest('ComfyGetGeneratedWorkflow', getGenInput(), (data) => {
        ...
        comfyFrame().contentWindow.app.loadApiJson(comfyPrepareWorkflowForSwarm(data.workflow));
    });
}
